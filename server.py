...
from flask import Flask, request, render_template # Import flask modules
from EmotionDetection.emotion_detection import emotion_detector # Import emotion detector module
...
def detector():
    ...
    # Take the text to analyze from request
    text_to_analyze = request.args.get("textToAnalyze")
    result = emotion_detector(text_to_analyze)

    # If dominant emotion is None then the text to analyze was not
    if result["dominant_emotion"] is None:
        message = "<strong>Invalid text! Please try again!</strong>"
        return message

    # Prepare message to show to user.
